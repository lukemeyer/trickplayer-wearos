...
class Settings(context: Context) {
    ...
    private val prefs =
        context.applicationContext.getSharedPreferences("trickplayer.settings", Context.MODE_PRIVATE)
        ...
        set(v) {
            prefs.edit().apply {
                if (v == null) {
                    ...
                    // A new episode invalidates the position AND everything
                    // derived from it.
                    //
                    // Resetting scene and cue alone is not enough, and that was
                    // a real bug: `timelineStartMs` still pointed at the burst
                    // pushed for the PREVIOUS episode, so the next complication
                    // request worked out how many steps had "played" since then
                    // and walked the brand-new episode forward by all of them.
                    // Picking something to watch started it somewhere in the
                    // middle.
                    putInt(K_SCENE, 0)
                    putInt(K_CUE, 0)
                    putLong(K_TL_START, 0L)     // no burst is in flight
                    putLong(K_LAST_ADVANCE, 0L) // the dwell gate starts open
                }
            }.apply()
        }
    ...
    var timelineHorizonMs: Long
        get() = prefs.getLong(K_TL_HORIZON, DEFAULT_AUTOPLAY_MS)
            .coerceIn(MIN_AUTOPLAY_MS, MAX_AUTOPLAY_MS)
        set(v) = prefs.edit()
            .putLong(K_TL_HORIZON, v.coerceIn(MIN_AUTOPLAY_MS, MAX_AUTOPLAY_MS))
            .apply()

    /**
     * How long an autoplay runs. The same number as [timelineHorizonMs], under
     * the name the user sees.
     *
     * **It wants to be shorter than the screen timeout**, which is the hint the
     * control carries: the horizon is not "how far ahead we could plan", it is
     * "how long the screen will be on", and those differ by two orders of
     * magnitude. Steps beyond the timeout are built, pushed across a Binder and
     * shown to a dark screen — and worse, they are then *credited as played*,
     * because elapsed wall-clock time is how the cursor catches up. Set it
     * longer than the timeout and the episode skips content nobody saw.
     */
    var autoplayLengthMs: Long
        get() = timelineHorizonMs
        set(v) { timelineHorizonMs = v }

    /**
     * When a tap-started autoplay ends, or 0.
     *
     * A tap cannot push a timeline itself — it can only ask the system to come
     * back and ask us — so "the user tapped and wants it to play" has to survive
     * until that request arrives.
     */
    var autoplayUntilMs: Long
        get() = prefs.getLong(K_AUTOPLAY_UNTIL, 0L)
        set(v) = prefs.edit().putLong(K_AUTOPLAY_UNTIL, v).apply()
    ...
    companion object {
         ...
         */
        const val SCENE_POLICY = "f001.f002.pad20d"
        ...
        /** The range the control offers, and what it clamps to. */
        const val MIN_AUTOPLAY_MS = 5_000L
        const val MAX_AUTOPLAY_MS = 30_000L
        const val DEFAULT_AUTOPLAY_MS = 15_000L

        const val K_AUTOPLAY_UNTIL = "autoplayUntilMs"
