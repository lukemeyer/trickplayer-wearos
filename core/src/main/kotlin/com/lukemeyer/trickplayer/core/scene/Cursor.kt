...
enum class Advance {
    ...
    /**
     * Keep going by itself for a few seconds, then stop.
     *
     * Not a fourth thing the cursor can do — **one step of an autoplay is
     * exactly a [NEXT_SUBTITLE] step**, which is why [Cursor] treats it as one.
     * What differs is how many steps happen per trigger and who drives them: a
     * run of them, on a timer, rather than one per glance.
     *
     * It earns a place beside the other three because it is what the Wear OS
     * face was always doing and never said (UI.md §4.2) — the burst was an
     * implementation detail leaking out as behaviour nobody had chosen.
     */
    AUTOPLAY,
}
...
class Cursor(
    ...
    fun advance(
        ...
        if (mode != Advance.NEXT_SCENE && cueIndex + 1 < cueCount) {
            cueIndex++
    ...
    companion object {
        ...
        fun walk(
            ...
            repeat(steps) {
                ...
                if (mode != Advance.NEXT_SCENE && cue + 1 < cues) {
                    cue++
                } else {
